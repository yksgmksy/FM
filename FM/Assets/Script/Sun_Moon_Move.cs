...
public class Sun_Moon_Move : MonoBehaviour {
	...
	void Update () {
        if ((StaticGlobal.gameTime.Hour >= 7 && StaticGlobal.gameTime.Hour <= 9) || (StaticGlobal.gameTime.Hour >= 16 && StaticGlobal.gameTime.Hour <= 18))
        {
            ...
            gameObject.transform.position = new Vector2(Mathf.Cos(Mathf.Deg2Rad * angle) * sunRadius + playerMove.MovingPostion.x,
                                                        Mathf.Sin(Mathf.Deg2Rad * angle) * sunRadius + playerMove.MovingPostion.y);
        }
